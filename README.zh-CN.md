...
面向 OpenAI 兼容 API 的多 Key、多端点批量可用性检测工具，支持按列盘点结果和实时延迟监控。
...
`check-your-api` 是一个基于 Web 的工具，用于用两种模式验证 OpenAI 兼容 API：同一个 Base URL 下的多枚 API Key，或同一枚 API Key 下的多个 Base URL。它会逐列拉取模型列表，合并模型并集，再按 Key 或 Endpoint 分组检测已列出的 `列 × 模型` 组合，提供状态、失败原因和首字延迟反馈。

**核心能力：**
- 从任何 OpenAI 兼容端点逐 Key 获取模型列表
- 使用同一枚 API Key 从多个 Base URL 获取模型列表
- 合并多枚 Key 或多个端点的模型并集
- 可配置总并发数的 `Key/Endpoint × 模型` 按列批量检测
- 首字延迟测量，提供性能洞察
- 模型选择和过滤，支持针对性测试
...
3. 前端合并所有成功返回的模型并集
4. 用户选择模型并启动批量检测
5. 前端按列轮询展开已列出的 `列 × 模型` 任务，并按总并发调用 `/api/check`
6. 后端代理流式请求到目标 API
7. 按 Key 或 Endpoint 分组显示可用性、首字延迟和失败原因
...
5. 部署完成后在生产域名上做一次模型拉取和批量检测 smoke test。
...
免费 Hobby 计划有函数调用量、运行时长和资源限制。批量检测由前端队列调度，建议总并发先从 3 到 5 开始；当任务数较大时，先用模型选择器缩小范围。
...
### 按列盘点
检测范围只包含该 Key 或端点的 `/models` 已列出的组合。某个选中模型没有被某一列列出时，不会在该列分组中显示，也不会向 `/api/check` 发起请求。

### 总并发控制
配置总并发数（1-N 个并行请求）以平衡速度、函数额度和上游速率限制。批量检测任务按列轮询排队，让多个 Key 或 Endpoint 更早同时出现进度。
...
- **按列语义**：每组只显示该 Key 或端点在 `/models` 中列出的模型；未列出的组合不会显示，也不会实际探测。
- **延迟**：首字延迟测量到第一个流式响应块的时间。
- **超时**：请求在 30 秒后超时。
- **大批量任务**：实际列出的 Key/端点与模型组合决定探测任务数。超过 100 个任务时界面会提示风险，但不会硬性阻止。
...
按列盘点的行为契约见 [`docs/multi-key-probe.md`](./docs/multi-key-probe.md)。
